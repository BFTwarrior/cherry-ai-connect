...
export function ClientKeyForm({ clientKey, providers, defaultLevel, language, onSubmit, actions }: {
  ...
  return <form onSubmit={onSubmit}>
    ...
    <label className="field-label">{tr("思考强度", "Reasoning level")}<select className="field-control" name="reasoningLevel" defaultValue={clientKey?.reasoningLevel || defaultLevel || "unchanged"}>{levels.map((level) => <option value={level} key={level}>{level.toUpperCase()}</option>)}</select><small className="field-help">{tr("选择 UNCHANGED 时，网关保留客户端请求原本的思考策略。", "Choose UNCHANGED to preserve the reasoning strategy supplied by the client.")}</small></label>
