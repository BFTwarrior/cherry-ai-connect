...
const emptyStatus: CloudSyncStatus = {
  enabled: false, connected: false, provider: "github", owner: "", repository: "cherry-ai-connect-sync", repositoryPrivate: false,
...
export function CloudSyncCard({ language, requestConfirmation }: { language: Language; requestConfirmation: ConfirmRequest }) {
  ...
  const connect = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!window.desktop?.connectGitHub) return setError(tr("请在桌面版中连接 GitHub", "Connect GitHub from the desktop app"));
    const formElement = event.currentTarget;
    const form = new FormData(formElement);
    const password = String(form.get("password") || "");
    const confirmPassword = String(form.get("confirmPassword") || "");
    if (password !== confirmPassword) return setError(tr("两次输入的加密密码不一致", "The vault passwords do not match"));
    if (password.length < 8) return setError(tr("加密密码至少需要 8 个字符", "Use at least 8 characters for the vault password"));
    setBusy(true); setError("");
    try {
      const result = await window.desktop.connectGitHub({ token: String(form.get("token") || ""), repository: String(form.get("repository") || "cherry-ai-connect-sync"), password });
  ...
  const syncNow = async () => {
    setBusy(true); setError("");
    try { if (window.desktop?.syncNow) setStatus(await window.desktop.syncNow()); }
    catch (reason) { setError(String(reason instanceof Error ? reason.message : reason)); }
    finally { setBusy(false); }
  };
  ...
  const toggle = async () => {
    setBusy(true); setError("");
    try { if (window.desktop?.setSyncEnabled) setStatus(await window.desktop.setSyncEnabled(!status.enabled)); }
    catch (reason) { setError(String(reason instanceof Error ? reason.message : reason)); }
    finally { setBusy(false); }
  };
  ...
  const disconnect = async () => {
    ...
    if (!confirmed) return;
    setBusy(true); setError("");
    try { if (window.desktop?.disconnectGitHub) setStatus(await window.desktop.disconnectGitHub()); }
    catch (reason) { setError(String(reason instanceof Error ? reason.message : reason)); }
    finally { setBusy(false); }
  };
  ...
  return <article className="settings-card cloud-sync-card">
    <div className="cloud-sync-heading"><span className="settings-icon cloud-icon"><SyncIcon name="cloud" /></span><div><div className="cloud-title-line"><h3>{tr("GitHub 云同步", "GitHub cloud sync")}</h3>{status.connected && <span className={`sync-state-chip state-${status.state.toLowerCase()}`}><span />{stateLabel}</span>}</div><p>{tr("使用一个私有仓库的固定 Release 保存加密备份；每 30 分钟及开启、关闭、启动、退出等重要时刻自动同步。", "Encrypted backups use one fixed Release in a private repository, syncing every 30 minutes and on major lifecycle events.")}</p></div></div>
    ...
    {!status.connected ? <form className="github-connect-form" onSubmit={connect}>
      ...
      <div className="sync-form-grid"><label><span>{tr("GitHub 访问令牌", "GitHub access token")}</span><input name="token" type="password" autoComplete="off" placeholder="github_pat_… / ghp_…" required /></label><label><span>{tr("私有仓库名称", "Private repository name")}</span><input name="repository" defaultValue="cherry-ai-connect-sync" pattern="[A-Za-z0-9._-]{1,100}" required /></label><label><span>{tr("备份加密密码", "Backup vault password")}</span><input name="password" type="password" minLength={8} autoComplete="new-password" required /></label><label><span>{tr("再次输入密码", "Confirm password")}</span><input name="confirmPassword" type="password" minLength={8} autoComplete="new-password" required /></label></div>
      <div className="sync-security-line"><SyncIcon name="shield" /><span>{tr("不会上传聊天内容、完整客户端 Key、密码、恢复码或 GitHub 令牌。线路地址与上游 Key 先加密再上传。", "Prompts, full client keys, passwords, recovery codes, and GitHub tokens never upload. Route URLs and upstream keys are encrypted first.")}</span></div>
